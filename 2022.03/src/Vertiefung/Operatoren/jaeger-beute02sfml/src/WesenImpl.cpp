...
void WesenImpl::ruhen(float dt) {
	...
	_farbe = sf::Color(100, 100, 100);
}

void WesenImpl::anzeigen() {
    sf::Color alpha(_farbe.r, _farbe.g, _farbe.b, 100);
    sf::CircleShape cs(_sichtradius);
    cs.setPosition(_position.x, _position.y);
    cs.setOrigin (_sichtradius, _sichtradius);
    cs.setFillColor(alpha);
    window->draw(cs);

    float radiusInnen = std::sqrt(_energie);
